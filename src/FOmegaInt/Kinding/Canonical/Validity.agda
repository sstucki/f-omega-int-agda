...
Sp⇉-valid (kd-Π j-kd k-kd) (⇉-∷ a⇇j _ k[a]⇉as⇉l) =
  Sp⇉-valid (TK.kd-/H k-kd (⇇-hsub [] a⇇j j-kd (⌊⌋-⌊⌋≡ _))) k[a]⇉as⇉l
...
kd-[] j-kd a⇇k = TK.kd-/H j-kd (⇇-hsub [] a⇇k (Nf⇇-valid a⇇k) (⌊⌋-⌊⌋≡ _))

Nf⇇-[] : ∀ {n} {Γ : Ctx n} {a b j k} →
         kd j ∷ Γ ⊢Nf a ⇇ k → Γ ⊢Nf b ⇇ j →
         Γ ⊢Nf a [ b ∈ ⌊ j ⌋ ] ⇇ k Kind[ b ∈ ⌊ j ⌋ ]
Nf⇇-[] a⇇k b⇇j = TK.Nf⇇-/H a⇇k (⇇-hsub [] b⇇j (Nf⇇-valid b⇇j) (⌊⌋-⌊⌋≡ _))

<∷-[≃] : ∀ {n} {Γ : Ctx n} {j k₁ k₂ a₁ a₂} →
         kd j ∷ Γ ⊢ k₁ <∷ k₂ → Γ ⊢ a₁ ≃ a₂ ⇇ j →
         Γ ⊢ k₁ Kind[ a₁ ∈ ⌊ j ⌋ ] <∷ k₂ Kind[ a₂ ∈ ⌊ j ⌋ ]
<∷-[≃] k₁<∷k₂ a₁≃a₂⇇j =
  TK.<∷-/H≃ k₁<∷k₂ (≃-hsub [] [] a₁≃a₂⇇j (⌊⌋-⌊⌋≡ _))

<:-[≃] : ∀ {n} {Γ : Ctx n} {a₁ a₂ b₁ b₂ j k} →
         kd j ∷ Γ ⊢ a₁ <: a₂ ⇇ k → Γ ⊢ b₁ ≃ b₂ ⇇ j →
         Γ ⊢ a₁ [ b₁ ∈ ⌊ j ⌋ ] <: a₂ [ b₂ ∈ ⌊ j ⌋ ] ⇇ k Kind[ b₁ ∈ ⌊ j ⌋ ]
<:-[≃] a₁<:a₂⇇k b₁≃b₂⇇j =
  TK.<:⇇-/H≃ a₁<:a₂⇇k (<:⇇-valid-kd a₁<:a₂⇇k) (≃-hsub [] [] b₁≃b₂⇇j (⌊⌋-⌊⌋≡ _))
