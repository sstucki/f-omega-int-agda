...
----------------------------------------------------------------------
-- Hereditary substitutions preserve the canonical judgments
--
-- NOTE.  The hereditary substitution lemma mutually depends on a
-- number of other lemmas, making for a total of 32 sub-lemmas
-- (including auxiliary lemmas) that all have to be proven
-- simultaneously.  The full dependency graph is unwieldy but it is
-- informative to list a few dependency cycles to illustrate the
-- dependencies between some of the high-level properties being
-- proved.
--
-- Cycle 1: preservation of the canonical judgments by hereditary
...
--   Nf⇇-Π-e          -->  Nf⇉-/⟨⟩          directly
--
-- Cycle 2: preservation of the canonical judgments by hereditary
-- substitution mutually depends on functionality of hereditary
-- substitution and reducing application.
--
--   kd-/⟨⟩           -->  Nf⇉-/⟨⟩          in case  kd-⋯
--   Nf⇉-/⟨⟩          -->  Ne∈-/⟨⟩          in case  ⇉-s-i
...
--   ≃-Π-e            -->  kd-/⟨⟩           in case  is-⇒
--
-- As one would expect, separate cycles also exist among the various
-- sub-lemmas of each of the high-level properties, and among the
-- properties and the auxiliary lemmas towards the end of the proof.
--
-- The whole glorious tangle of definitions terminates because each
-- call either structurally recurses on the shape parameter `k' of the
-- hereditary substitutions and reducing applications involved or it
-- passes on the same `k' but recurses on one of the derivations.  In
-- some cases, the latter happens indirectly through a short chain of
-- (non-recursive) calls.  In other words, this is a proof by
-- structural induction on the syntax of the shape parameter `k' and
-- inner/local inductions on the derivations of the various judgments
-- involved.
